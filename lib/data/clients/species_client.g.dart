...
class _SpeciesClient implements SpeciesClient {
  ...
  Future<ConservationMeasuresResponseModel> getConservationMeasuresPerName({
    ...
    final Map<String, dynamic>? _data = null;
    final _result = await _dio.fetch<Map<String, dynamic>>(
        _setStreamType<ConservationMeasuresResponseModel>(Options(
      method: 'GET',
      headers: _headers,
      extra: _extra,
    )
            .compose(
              _dio.options,
              '/measures/species/name/${name}/region/${region}?token=${token}',
              queryParameters: queryParameters,
              data: _data,
            )
            .copyWith(baseUrl: baseUrl ?? _dio.options.baseUrl)));
    final value = ConservationMeasuresResponseModel.fromJson(_result.data!);
    return value;
  }
